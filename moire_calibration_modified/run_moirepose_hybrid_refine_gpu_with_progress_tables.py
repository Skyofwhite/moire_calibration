"""MoiréPose hybrid refinement runner (GPU match + LM refine) + table-style summaries.

This is a *drop-in* extended version of your original runner that:
  1) keeps the per-pair XLSX logging (same as before)
  2) additionally writes summary sheets that are easy to convert into:
     - HPatches Table-2 style: AUC@{3,5,10}px on a chosen error metric
     - InLoc/Aachen Table-3/4 style: Success% at multiple thresholds

Key idea:
  - You already log per-pair pixel errors (hybrid_rms_before_px / hybrid_rms_after_px).
  - From those, you can compute:
      AUC@T = mean( max(0, 1 - err/T) ) * 100
    and
      Success@T = mean( err <= T ) * 100
  - Failures (e.g., too few inliers) are treated as err = +inf => 0 contribution.

Usage (same as original):
  python run_moirepose_hybrid_refine_gpu_with_progress_tables.py --root <DATA_ROOT> --matcher loftr --patch_mode

New knobs:
  --auc_thresholds   "3,5,10"  (comma-separated)
  --succ_thresholds  "3,5,10"  (comma-separated)

NOTE:
  If you want to reproduce *actual* HPatches/InLoc/Aachen benchmarks, use dedicated
  benchmark runners (see eval_hpatches_auc.py, and HLoc pipeline for localization).
  This file focuses on producing *the same table format* for your own MoiréPose experiment.
"""
...
# -----------------------------
# Summary metrics (Table-2 / Table-3/4 style)
# -----------------------------
...
def auc_at(errors_px: np.ndarray, thr_px: float) -> float:
    """Exact AUC of recall-vs-threshold curve, normalized to [0,100]."""
    e = np.asarray(errors_px, dtype=np.float64)
    return float(np.mean(np.clip(1.0 - (e / float(thr_px)), 0.0, 1.0)) * 100.0)


def success_at(errors_px: np.ndarray, thr_px: float) -> float:
    e = np.asarray(errors_px, dtype=np.float64)
    return float(np.mean(e <= float(thr_px)) * 100.0)


def parse_thresholds(csv: str) -> List[float]:
    out: List[float] = []
    for t in csv.split(","):
        t = t.strip()
        if not t:
...
def group_summaries(rows: List[Dict], group_keys: Tuple[str, ...], auc_thrs: List[float], succ_thrs: List[float]):
    """Return list of summary dicts, one per group."""

    # Only summarize evaluated pairs (exclude missing original rows)
    rows_eval = [r for r in rows if r.get("orig_path")]
    ...
    for key, rs in sorted(buckets.items(), key=lambda kv: kv[0]):
        # error metric: hybrid_rms_after_px
        errs = []
        errs_before = []
        ok_flags = []
        t_total = []
        ...
        for r in rs:
            ...
            e_after = _safe_float(r.get("hybrid_rms_after_px"))
            e_before = _safe_float(r.get("hybrid_rms_before_px"))

            # Treat failures or missing values as +inf so they contribute 0.
            if not np.isfinite(e_after) or ok == 0:
                e_after = float("inf")
            if not np.isfinite(e_before) or ok == 0:
                e_before = float("inf")

            errs.append(e_after)
            errs_before.append(e_before)

            tt = _safe_float(r.get("t_total"))
            tm = _safe_float(r.get("t_match"))
            if np.isfinite(tt):
                t_total.append(tt)
            if np.isfinite(tm):
                t_match.append(tm)
        ...
        ok_flags = np.array(ok_flags, dtype=np.int64)

        s: Dict = {k: v for k, v in zip(group_keys, key)}
        s.update({
            "n_pairs": int(len(rs)),
            "success_inlier_%": float(np.mean(ok_flags) * 100.0),
            "mean_t_total_s": float(np.mean(t_total)) if t_total else None,
            "mean_t_match_s": float(np.mean(t_match)) if t_match else None,
        })

        for thr in auc_thrs:
            s[f"AUC@{thr:g}px_after"] = auc_at(errs, thr)
            s[f"AUC@{thr:g}px_before"] = auc_at(errs_before, thr)

        for thr in succ_thrs:
            s[f"Succ@{thr:g}px_after_%"] = success_at(errs, thr)

        # Optional: mean RMS among successes only
        ok_errs = np.array([_safe_float(r.get("hybrid_rms_after_px")) for r in rs if int(r.get("ok", 0) == 1)], dtype=np.float64)
        ok_errs = ok_errs[np.isfinite(ok_errs)]
        s["mean_rms_after_px_ok"] = float(np.mean(ok_errs)) if ok_errs.size else None
...
def find_augmented(aug_dir: Path) -> List[Tuple[str, str, str, str, Path]]:
    ...
    for p in aug_dir.iterdir():
        if p.is_file():
            ...
            if m:
                items.append((m.group("idx"), m.group("base").lower(), m.group("augtype").lower(), m.group("k"), p))
...
# ORB matching (CPU)
# -----------------------------
def orb_match_points(g0: np.ndarray, g1: np.ndarray, nfeatures: int = 3000, max_matches: int = 1500):
    ...
    if des0 is None or des1 is None or nkp0 < 30 or nkp1 < 30:
        return (np.zeros((0, 2), np.float64), np.zeros((0, 2), np.float64), nkp0, nkp1, 0)
...
class LoFTRMatcher:
    ...
    def match(self, g0: np.ndarray, g1: np.ndarray, max_side: int = 640):
        """Returns (mkpts0, mkpts1, nmatch, s0, s1) in ORIGINAL crop pixel coordinates."""
...
def crop_intrinsics_from_params(params: dict, crop_wh: Tuple[int, int]) -> np.ndarray:
    ...
    w, h = crop_wh
    cx, cy = w / 2.0, h / 2.0
    return np.array([[f_px, 0.0, cx], [0.0, f_px, cy], [0.0, 0.0, 1.0]], dtype=np.float64)
...
def refine_aug_pose_hybrid(
    ...
    if matcher == "loftr":
        ...
        pts0, pts1, nkp0, nkp1, nmatch = orb_match_points(g0, g1, nfeatures=orb_nfeatures, max_matches=orb_max_matches)
    t3 = time.time()
    ...
    if not isinstance(pose_aug, dict):
        return {"ok": False, "reason": "aug_res.pose missing/invalid", "pair": pair}
...
def main():
    ...
    ap.add_argument("--auc_thresholds", default="3,5,10", help="Comma-separated px thresholds (AUC)")
    ap.add_argument("--succ_thresholds", default="3,5,10", help="Comma-separated px thresholds (Success%)")

    args = ap.parse_args()

    auc_thrs = parse_thresholds(args.auc_thresholds)
    succ_thrs = parse_thresholds(args.succ_thresholds)
    ...
    print(f"[INFO] summaries: AUC@{auc_thrs}px  Success@{succ_thrs}px")
    ...
    for (idx, base, augtype, k, aug_path) in tqdm(augmented, desc="MoiréPose Hybrid (GPU match + LM refine)", unit="pair"):
        ...
        if orig_path is None:
            rows.append({
                "ok": 0,
                "patch_mode": int(args.patch_mode),
                "idx": idx,
                "base": base,
                "augtype": augtype,
                "aug_k": k,
        ...
        try:
            ...
            if not out.get("ok", False):
                rows.append({
                    ...
                    "matcher": out.get("matcher", args.matcher),
    ...
    # -----------------------------
    # Summary sheets (Table-2 / Table-3/4 style)
    # -----------------------------
    # 1) Overall by (matcher, patch_mode)
    summ1 = group_summaries(rows, group_keys=("matcher", "patch_mode"), auc_thrs=auc_thrs, succ_thrs=succ_thrs)
    ws1 = wb.create_sheet("summary_overall")
    write_sheet(ws1, summ1)

    # 2) By base (DUC1/DUC2 or Day/Night analog)
    summ2 = group_summaries(rows, group_keys=("matcher", "patch_mode", "base"), auc_thrs=auc_thrs, succ_thrs=succ_thrs)
    ws2 = wb.create_sheet("summary_by_base")
    write_sheet(ws2, summ2)

    # 3) By augmentation type
    summ3 = group_summaries(rows, group_keys=("matcher", "patch_mode", "base", "augtype"), auc_thrs=auc_thrs, succ_thrs=succ_thrs)
    ws3 = wb.create_sheet("summary_by_augtype")
    write_sheet(ws3, summ3)
