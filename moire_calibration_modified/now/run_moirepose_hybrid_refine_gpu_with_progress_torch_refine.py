...
# ============================================================
# Torch-based GPU refinement (reprojection loss)
# ============================================================
def _torch_require():
    if torch is None:
...
def refine_pose_gpu_torch(
    obj_pts_np: np.ndarray,
    img_pts_np: np.ndarray,
    K_np: np.ndarray,
    rvec0_np: np.ndarray,
    tvec0_np: np.ndarray,
    device: str = "cuda",
    ...
    """
    Returns refined (rvec, tvec, final_loss) as numpy arrays + scalar loss.
    """
    _torch_require()

    if device.startswith("cuda") and (not torch.cuda.is_available()):
        raise RuntimeError("Torch refine requested on CUDA but torch.cuda.is_available() is False.")

    dev = torch.device(device)
    dtype = torch.float32

    X = torch.from_numpy(obj_pts_np).to(dev, dtype=dtype)
    u_obs = torch.from_numpy(img_pts_np).to(dev, dtype=dtype)
    ...
    if method == "adam":
        opt = torch.optim.Adam([w, t], lr=5e-3)
        for _ in range(iters):
            opt.zero_grad(set_to_none=True)
            R = _rodrigues(w)
            u_pred = _project_torch(Kt, R, t, X)
            res = u_pred - u_obs
            loss = _huber_loss(res, delta=huber_delta)
            loss.backward()
            opt.step()
            final_loss = loss.detach()
    else:
        opt = torch.optim.LBFGS([w, t], lr=1.0, max_iter=iters, line_search_fn="strong_wolfe")
    ...
    return rvec_ref, tvec_ref, loss_val
...
def refine_aug_pose_hybrid(
    ...
    refine_device: str,           # "cuda" or "cpu"
    torch_iters: int,
    torch_method: str,            # "lbfgs" or "adam"
    huber_delta: float
) -> Dict:
    t_total0 = time.time()
    ...
    if matcher == "loftr":
        if loftr is None:
            return {"ok": False, "reason": "matcher=loftr but LoFTR not available (install kornia/torch)", "pair": pair}
    ...
    # 7) Refinement (OpenCV CPU or Torch GPU)
    t5 = time.time()
    torch_final_loss = None

    if refine_backend == "torch":
        try:
            rvec_ref, tvec_ref, torch_final_loss = refine_pose_gpu_torch(
                obj_pts_np=obj_pts,
                img_pts_np=img_pts,
                K_np=Kmat,
                rvec0_np=rvec0,
            ...
            return {"ok": False, "reason": f"torch_refine_failed: {type(e).__name__}: {e}", "pair": pair}
    else:
        # OpenCV LM refinement (CPU)
    ...
    return {
        ...
        "refine_device": refine_device if refine_backend == "torch" else None,
        "torch_method": torch_method if refine_backend == "torch" else None,
        "torch_iters": torch_iters if refine_backend == "torch" else None,
        "huber_delta": huber_delta if refine_backend == "torch" else None,
        "torch_final_loss": torch_final_loss,
...
def main():
    ...
    # LoFTR params
    ap.add_argument("--device", default="cuda")  # "cuda" or "cpu" for LoFTR
    ...
    ap.add_argument("--refine_device", default="cuda")  # torch refine device
    ap.add_argument("--torch_iters", type=int, default=25)
    ap.add_argument("--torch_method", default="lbfgs", choices=["lbfgs", "adam"])
    ap.add_argument("--huber_delta", type=float, default=3.0)

    args = ap.parse_args()

    # Setup LoFTR matcher if needed
    loftr = None
    ...
    # Validate torch refine requirements early
    if args.refine_backend == "torch":
        if torch is None:
            raise RuntimeError("refine_backend=torch requested but torch is not installed.")
        if args.refine_device.startswith("cuda") and (not torch.cuda.is_available()):
    ...
    print(f"[INFO] matcher={args.matcher} device={args.device} loftr_max_side={args.loftr_max_side}")
    print(f"[INFO] refine_backend={args.refine_backend} refine_device={args.refine_device} torch_method={args.torch_method} torch_iters={args.torch_iters}")

    rows: List[Dict] = []
    for (idx, base, augtype, k, aug_path) in tqdm(augmented, desc="MoiréPose Hybrid (GPU match + refine)", unit="pair"):
        ...
        try:
            ...
            rows.append({
                ...
                "refine_backend": out.get("refine_backend"),
                "refine_device": out.get("refine_device"),
                "torch_method": out.get("torch_method"),
                "torch_iters": out.get("torch_iters"),
                "huber_delta": out.get("huber_delta"),
